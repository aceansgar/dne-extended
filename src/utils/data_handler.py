...
class DataHandler(object):
    ...
    @staticmethod
    def load_unweighted_digraph_pad_test(params):
        G = nx.DiGraph()
        with open(params["network_file"], "r") as f:
            for line in f:
                line = line.strip()
    ...
    @staticmethod
    def out_degree_distribution_omit_neg(G, params = None, num_to_del=0):
        num_remain = G.number_of_nodes() - num_to_del
        ret = [0.0] * G.number_of_nodes()
        for u in G:
            ret[u] = G.node[u]['out_degree']
        return ret[:num_remain]

    @staticmethod
    def in_degree_distribution_omit_neg(G, params = None, num_to_del=0):
        num_remain = G.number_of_nodes() - num_to_del
        ret = [0.0] * G.number_of_nodes()
        for u in G:
            ret[u] = G.node[u]['in_degree']
