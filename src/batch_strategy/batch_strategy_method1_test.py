...
class BatchStrategy(object):
    ...
    def __init__(self, G, num_to_del, mapp, rmapp, num_modify, params = None):
        ...
        for eid in range(delta_edge_num):
            ...
            if tmp_edge[0] < num_remain and tmp_edge[1] < num_remain:
                ...
                continue
                # self.edges_neg_neg.append(tmp_edge)
                # probs_in_neg_neg.append(probs_in[eid])
                # probs_out_neg_neg.append(probs_out[eid])
            else:
        ...
        # self.sampling_handler_in_neg_neg = at(probs_in_neg_neg)
        # self.sampling_handler_out_neg_neg = at(probs_out_neg_neg)
    ...
    def get_batch(self, batch_size):
        ...
        for a in xrange(batch_size_neg):
            ...
            # e_id = self.sampling_handler_in_neg_neg.sample()
            # batch_x_in_neg_neg.append(self.edges_neg_neg[e_id][0])
            # batch_y_in_neg_neg.append(self.edges_neg_neg[e_id][1])
            # e_id = self.sampling_handler_out_neg_neg.sample()
            # batch_x_out_neg_neg.append(self.edges_neg_neg[e_id][0])
            # batch_y_out_neg_neg.append(self.edges_neg_neg[e_id][1])
