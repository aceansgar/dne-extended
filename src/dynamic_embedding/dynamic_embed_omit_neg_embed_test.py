...
method 2
weight is context
unigrams_in in method 2 is not none,if unigrams_in is problematic.
batch strategy might be problematic,for that edges with two deleting nodes might influence embed.
'''
...
class NodeEmbedding(object):
    def __init__(self, params, init_embeddings, init_weights, G, mapp, rmapp, num_to_del = 1):
        ...
        self.lbd_remain = params["lambda_remain"]
        ...
        with self.tensor_graph.as_default():
            ...
            self.w = tf.concat([self.constant_weights, self.modify_weights, self.constant_weights_to_del], 0)
            ...
            self.total_rem_id = tf.constant(range(self.num_remain))
            self.delta_embeddings_remain = tf.nn.embedding_lookup(self.delta_embeddings_pad, self.total_rem_id)
            self.delta_weights_remain = tf.nn.embedding_lookup(self.delta_weights_pad, self.total_rem_id)
            ...
            self.loss_neg = 0.1* self.loss_in_neg
            self.loss_reg_batch = self.lbd_batch * (tf.norm(self.delta_embeddings_batch) + tf.norm(self.delta_weights_batch))
            self.loss_reg_remain = self.lbd_remain * (tf.norm(self.delta_embeddings_remain) + tf.norm(self.delta_weights_remain))
            #self.loss = self.loss_pos - tf.nn.sigmoid(self.loss_neg) + self.loss_reg
            self.loss = self.loss_pos - self.loss_neg + self.loss_reg_batch + self.loss_reg_remain
        ...
        with tf.Session(graph = self.tensor_graph) as sess:
            ...
            for i in xrange(self.epoch_num):
                ...
                if (i % 1000 == 0):
                    ...
                    # weight_batch_neg = self.weight_batch_neg.eval(
                    #     {
                    #         self.x_in: batch_x_in,
                    #         self.x_out: batch_x_out,
                    #         self.labels_in: batch_labels_in,
