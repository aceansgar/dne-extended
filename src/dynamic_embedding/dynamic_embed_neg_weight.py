...
class NodeEmbedding(object):
    def __init__(self, params, init_embeddings, init_weights, G, mapp, rmapp, num_neg_new = 1):
        self.num_nodes_init, self.embedding_size = init_embeddings.shape
        self.num_nodes = self.num_nodes_init + num_neg_new
        self.num_remain = self.num_nodes_init - num_neg_new
        ...
        self.bs = __import__("batch_strategy." + params["batch_strategy"]["func"], fromlist = ["batch_strategy"]).BatchStrategy(G, num_neg_new, mapp, rmapp, self.num_modify, params["batch_strategy"])
        ...
        with self.tensor_graph.as_default():
            ...
            self.constant_weights_to_del = tf.constant(init_weights[self.num_remain: self.num_nodes_init])
            ...
            self.new_embeddings = tf.Variable(tf.random_uniform([num_neg_new, self.embedding_size], -1.0, 1.0), dtype = tf.float32)
            self.new_weights = tf.Variable(tf.random_uniform([num_neg_new, self.embedding_size], -1.0, 1.0), dtype = tf.float32)
            self.nce_biases = tf.zeros([self.num_nodes], tf.float32)
            self.nce_biases_no_neg = tf.zeros([self.num_remain + num_neg_new], tf.float32)


            self.embed = tf.concat([self.constant_embeddings, self.modify_embeddings, self.constant_embeddings_to_del, self.new_embeddings], 0)
            self.w = tf.concat([self.constant_weights, self.modify_weights, self.constant_weights_to_del, self.new_weights], 0)
            self.w_no_neg =tf.concat([self.constant_weights, self.modify_weights, self.constant_weights_to_del])
            ...
            self.delta_embeddings_pad = tf.concat(
                    [tf.zeros([self.num_remain - self.num_modify, self.embedding_size], dtype = tf.float32),
                        ...
                        tf.zeros([2*num_neg_new, self.embedding_size], dtype = tf.float32)],
                    axis = 0)
