'''
let the small node on the left of the edge
'''
import os
import sys
import re
...
def main():
    ...
    with open(nw_file, "r") as f:
        for line in f:
            ...
            if len(items) == 1:
                ...
                for node_id in xrange(n, node_num):
                    G_dynamic[node_id] = []
            if len(items) != 2:
                continue
    ...
    dynamic_nw_file_reverse=os.path.join(DATA_PATH, args.input_file + "_" + str(n)  + "_" + str(args.ratio) +
                                             "_nw_dynamic_reverse")
    ...
    tmp.reverse()
    with open(dynamic_nw_file_reverse, "w") as f:
        for u, s in tmp:
            f.write(str(u) + "\t" + str(len(s)) + "\n")
            for v, w in s:
                f.write(str(v) + "\t" + str(w) + "\n")
